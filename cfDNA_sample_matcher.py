...
def get_genes(df):
    ...
    for row in df.iterrows():
        index, data = row
        geneName = data['reportedVariantCancer.reportEvents'][0]['genomicFeatureCancer']['geneName']
        if bool(geneName in set(thermo_genes)):
            chromosome = data['reportedVariantCancer.chromosome']
            position = data['reportedVariantCancer.position']
            ensemblId = data['reportedVariantCancer.reportEvents'][0]['genomicFeatureCancer']['ensemblId']
            cDNAchange = data['reportedVariantCancer.cDNAchange']
            proteinChange = data['reportedVariantCancer.proteinChange']
            vaf = data['reportedVariantCancer.VAF']
            # gene_details_list.append(','.join((geneName, (':'.join((str(chromosome), str(position)))), ensemblId, cDNAchange, proteinChange, str(vaf))))
            gene_details_list.append(','.join((str(geneName), ("chr" + str(chromosome) + ":" + str(position)), str(ensemblId), str(cDNAchange), str(proteinChange), str(vaf))))
    gene_details = ';'.join(filter(None, np.asarray(gene_details_list)))
    return gene_details
...
def check_sample_for_genes(json_files_df):
    ...
    if (get_genes(json_df_som)):
        gene_match = True
        gene_matches = get_genes(json_df_som)
